...
course_plugin = lightbulb.Plugin("all", 'Get all course assignment embeds in server')

@course_plugin.command
@lightbulb.command(
    "all", 'Get all course assignment embeds in server'
)
@lightbulb.implements(lightbulb.SlashCommand)
async def all(ctx: lightbulb.Context):
    # get server id
    guild_id = ctx.guild_id
    server__all_reqs = Firestore().get_server_requests(str(guild_id))
